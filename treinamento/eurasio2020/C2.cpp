...
set<int> solutions[MAXN];
int degree[MAXN];
set<int> auxiliar;

void construct_solution(int u){
    if (!degree[u]){
        solutions[u].insert(0);
        solutions[u].insert(1<<u);
        return;
    }
    auxiliar.clear();
    ...
    for (auto v:neighbor[u]){
        ...
        for (auto s1:solutions[u]){
            for (auto s2:solutions[v]){
                auxiliar.insert(s1|s2);
            }
        }
        for (auto s:auxiliar){
            solutions[u].insert(s);
        }
        auxiliar.clear();
    }
}
...
void solve(){
    ...
    //sort(all(solutions[0]), compare);
    auto it1 = solutions[0].begin();
    auto it2 = it1;
    it2++;
    for (;it2!=solutions[0].end(); it2++){
        cout << bitset<10>((*it2)^(*it1)) << endl;
        it1 = it2;
    }  
    cout << "////////////\n";
