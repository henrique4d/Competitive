...
namespace sparse {
    ...
    ll n;
	void build(int n2, pair<ll,ll>* v) {
		n = n2;
...
void solve(){
    ...
    for (int i=0; i<n; i++){
        cin >> a[i].first;
        a[i].second = i;
        aux.push_back({a[i].first,i});
    }
    ll mais_direita = -1;
    sort(all(aux));
    for (auto [h,pos]:aux){
        // cout << h << "-" << pos << endl;
        if (h > mais_direita) direita[pos] = h;
        else direita[pos] = mais_direita;
        mais_direita = max(mais_direita, h);
    }
    for (int i=0; i<n; i++){
        cout << i << " " << direita[i] << endl;
    }
    sparse::build(n, a);

    for (int i=0; i<n; i++){
        auto [_,pos] = sparse::query(0,i);
        cout << direita[pos]+1 << " ";
        // cout << i << " " << pos << endl;
        // ans[i] = direita[pos];
    }
