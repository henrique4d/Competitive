...
void solve(){
    ...
    cin >> a >> b >> c >> d;
    for (auto it:f){
        ll iniciox = (a+it.first-1)/it.first;
        ll fimx = b/it.first;
        if (fimx < iniciox) continue;

        ll inicioy = (c + it.second-1)/it.second;
        ll fimy = d/it.second;
        if (fimy < inicioy) continue;

        ll inicio = max(iniciox, inicioy);
        ll fim = min(fimx, fimy);
