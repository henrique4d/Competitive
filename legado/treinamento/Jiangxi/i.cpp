...
void solve(){
    ...
    for (int i=0; i<n; i++){
        for (int j=0; j<n; j++){
            pt c2;
            c2.x = (pontos[i].x + pontos[j].x)/2.0;
            c2.y = (pontos[i].y + pontos[j].y)/2.0;

            ll cont2 = 0;

            for (int k=0; k<n; k++){
                if ( dist2(c2, pontos[k])-dist2(c2, pontos[i]) <= eps) cont2++;
                pt c1 = getcenter(pontos[i], pontos[j], pontos[k]);
                ll cont = 0;
                for (int z = 0; z<n; z++){
                    if ( dist2(c1,pontos[z]) - dist2(pontos[i], c1) <= eps) cont++;
                }
                resp[(cont*(cont-1))/2.0] = min (resp[(cont*(cont-1))/2.0], dist(c1,pontos[i]));
            }
            resp[(cont2*(cont2-1))/2.0] = min (resp[(cont2*(cont2-1))/2.0], dist(c2,pontos[i]));
        }
    }

    
    for (int i=(n*(n-1))/2; i>=0; i--){
        resp[i] = min(resp[i], resp[i+1]);
    }
    for (int i=1; i<=(n*(n-1))/2; i++){
        cout << fixed << setprecision(20) << resp[i] << endl;
    }
