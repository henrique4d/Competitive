...
void build(int v, int tl, int tr){
    ...
    if (tl == tr){
        ...
        // cout << tl << " " << st[v].val << " - " << st[v].cont << endl;
        // cout << v << " " << tl << " " << st[v].cont << endl;
        return;
    }
    int tm = tl + (tr - tl)/2;
    build(2*v, tl, tm);
    ...
    //cout << "o valor e " << v << " " << tl << " " << tr << " " << st[v].cont << endl;
}

ll query(int v, int tl, int tr, int l, int r){
    //cout << "chamando query para " << v << " " << tl << " " << tr << " " << l << " " << r << endl;
    if (l > r) return 0;
    st[v].d -= lazy[v];
    push(v);
    if (tl == l and tr == r) return st[v].cont;
    int tm = tl + (tr - tl)/2;
...
void update(int v, int tl, int tr, int l, int r, int val){
    if (l > r) return;
    //cout << "chamando funcao de update " << v << " " << tl << " " << tr << " " << l << " " << r << " " << val << endl;
    if (l <= tl and r >= tr){
        if (tl == tr){
            st[v].val += lazy[v] + val;
            ...
            lazy[v] = 0;
            //cout << "depois de att " << v << " " << tl << " " << st[v].val << " " << st[v].d << " " << st[v].cont << endl;
            return;
        }
